(function($) {
    ...
    $.fn.formHandler = function(options) {
        ...
        _self.submit(function(_event) {
            ...
            if (!errors.length && options.type in formActions) {
                ...
                if (typeof options.errorModal !== 'undefined' && options.errorModal) {
                    ...
                    } else if (app_config.show_validation_errors == 'hide') {} else {}
                    errors = [];
                }
            }
        });
        //Check fields already filled
        $('input.formfield[type=text], input.formfield[type=email],input.formfield[type=tel]').each(
            function(){
                var val = $(this).val();
                if (val != '' || val != ''){
        ...
        $('select.formfield, select.formselect').each(
            function(){
		...
		_self.find('input.required[type=text],input.required[type=email],input.required[type=tel], select.required, textarea.required').blur(function (e) {    
			if (!$(this).val().length) {
        ...
        function submitForm() {
            ...
            $.ajax({
                url: '../app/ajax.php?method=' + actionType + '&crm=' +crm,
                method: 'post',
			...
			// console.log("2: responseCode was 101");
                        ...
                        if (typeof options.onError === 'function') {
                            // console.log("3: calling a function to handle error");
                            options.onError(data);
                        } else {
                            try {
                                if ($(options.responseLoader.div).length) {
                                    // console.log("here again in else with div");
        ...
        function getCountries(country, state) {
            ...
            $.ajax({
                url: app_config.offer_path + 'assets/common/storage/country_states.json',
                dataType: 'json',
                global: false,
        ...
        function getStates(state, country) {
            ...
            $.ajax({
                url: app_config.offer_path + 'src/common/storage/country_states.json',
                dataType: 'json',
                global: false,
                success: function(json) {
                    var cid = $(parent).val() + '-';
